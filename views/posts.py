...
class PostListEndpoint(Resource):
    ...
    def get(self):
        ...
        if limit > 50: return Response(json.dumps({"message": "limit must be at most 50"}), mimetype="application/json", status=400)
    ...
    def post(self):
        ...
        if not body.get('image_url'): return Response(json.dumps({"message": "image url is required"}), mimetype="application/json", status=400)
...
class PostDetailEndpoint(Resource):
    ...
    def patch(self, id):
        ...
        if not post: return Response(json.dumps({"message": "post id id={0} is invalid"}), mimetype="application/json", status=404)
        if post.user_id != self.current_user.id: return Response(json.dumps({"message": "post id id={0} is invalid"}), mimetype="application/json", status=404)
    ...
    def get(self, id):
        ...
        user_ids = get_authorized_user_ids(self.current_user)
        if post.user_id not in user_ids: return Response(json.dumps({"message": "post id id={0} is invalid"}), mimetype="application/json", status=404)
